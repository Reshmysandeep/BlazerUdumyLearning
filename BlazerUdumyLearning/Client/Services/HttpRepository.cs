...
namespace BlazerUdumyLearning.Client.Services
{
    public class HttpRepository<T> : IDisposable, IHttpRepository<T> where T : class
    {
        ...
        public HttpRepository(HttpClient client, HttpInterceptorService interceptorService)
        {
            _client = client;
            _interceptor = interceptorService;
        }
